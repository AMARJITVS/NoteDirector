...
class VideoFragment : ViewPagerFragment(), SurfaceHolder.Callback, SeekBar.OnSeekBarChangeListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        mView = inflater.inflate(com.amar.NoteDirector.R.layout.pager_video_item, container, false)
        setupPlayer()
    ...
    private fun setupPlayer() {
        ...
        mView.video_volume_controller.setOnTouchListener { _, event ->
            handleVolumeTouched(event)
            true
        }

        mView.video_brightness_controller.setOnTouchListener { _, event ->
    ...
    private fun handleVolumeTouched(event: MotionEvent) {
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                mSlideInfoText = "${getString(com.amar.NoteDirector.R.string.volume)}:\n"
            }
    ...
    private fun handleBrightnessTouched(event: MotionEvent) {
        when (event.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                mLastTouchY = event.y
                mTouchDownTime = System.currentTimeMillis()
                mSlideInfoText = "${getString(com.amar.NoteDirector.R.string.brightness)}:\n"
    ...
    private fun initTimeHolder() {
        ...
        var right = res.getDimension(com.amar.NoteDirector.R.dimen.timer_padding).toInt()
        var bottom = 0
    ...
    private fun pauseVideo() {
        mIsPlaying = false
        mMediaPlayer?.pause()
        mView.video_play_outline.setImageDrawable(resources.getDrawable(com.amar.NoteDirector.R.drawable.img_play_outline_big))
    ...
    private fun initMediaPlayer() {
        ...
        try {
            mMediaPlayer = MediaPlayer().apply {
                ...
                setOnVideoSizeChangedListener({ _, _, _ -> setVideoSize() })
