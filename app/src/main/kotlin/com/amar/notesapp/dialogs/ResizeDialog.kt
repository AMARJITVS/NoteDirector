...
class ResizeDialog(val activity: SimpleActivity, val size: Point, val callback: (newSize: Point) -> Unit) {
    init {
        val view = LayoutInflater.from(activity).inflate(com.amar.NoteDirector.R.layout.resize_image, null)
        ...
        AlertDialog.Builder(activity)
            ...
            activity.setupDialogStuff(view, this, com.amar.NoteDirector.R.string.resize_and_save)
            getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener({
                val width = getViewValue(widthView)
                val height = getViewValue(heightView)
                if (width <= 0 || height <= 0) {
                    activity.toast(com.amar.NoteDirector.R.string.invalid_values)
